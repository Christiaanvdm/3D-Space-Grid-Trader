...
using Mirror;
    ...
    public class ShipInput : NetworkBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (!DisableSteering && base.isLocalPlayer)
            {
        ...
        public override void OnStartLocalPlayer()
        {
            ...
            //mainCam = GetComponentInChildren<CameraRig>().cam;
        ...
        //    GameObject cameraObj = GameObject.Instantiate<GameObject>(Resources.Load("CameraPrefab") as ); // add camera
        ...
        private void SetStickCommandsUsingAutopilot()
        {
            ...
            mousePos.z = 1000f;
            Vector3 gotoPos = mainCam.ScreenToWorldPoint(mousePos);
			...
			BankShipRelativeToUpVector(mousePos, mainCam.transform.up);
		}
		...
		void Cmdshoot()
		{
			...
			GameObject new_bullet = Instantiate(Resources.Load<GameObject>("Bullet"), transform.position, transform.rotation);
			new_bullet.transform.localPosition = new Vector3(0f, 0, 10f);
            new_bullet.transform.position = transform.position;
			//new_bullet.gameObject.layer = current_layer;
			NetworkServer.Spawn(new_bullet);
            new_bullet.GetComponent<Rigidbody>().velocity = rb.velocity + transform.forward * 100f;
        ...
        //public void SetCamera(GameObject _cameraGO)
        //{
        //    cameraGO = _cameraGO;
        //    Camera.SetupCurrent(cameraGO.GetComponent<Camera>());
        //}
